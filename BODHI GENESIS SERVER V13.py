                    ...
                    if self.cache['last_update']:
                        try:
                            ...
                            if (now - update_time).seconds > 3600:
                                ...
                                logger.warning("[SENTIMENT] Data is stale (>1h old)")
                        except:
                            pass
        except Exception as e:
            logger.warning(f"[SENTIMENT] Error reading file: {e}")
...
class KarmaEngine:
    ...
    def _load(self):
        ...
        def _save(self):
            ...
            try:
                # Atomic write
                dir_name = os.path.dirname(self.karma_file)
                with tempfile.NamedTemporaryFile('w', dir=dir_name, delete=False) as tf:
                    json.dump(self.karma, tf, indent=2, default=str)
                    temp_name = tf.name
                shutil.move(temp_name, self.karma_file)
            except Exception as e:
